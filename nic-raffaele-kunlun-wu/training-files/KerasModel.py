...
class KerasRegressor(BaseEstimator, RegressorMixin):
    ...
    def _build_model(self, input_dim):
        ...
        x = layers.Dense(self.units_1, activation="relu", kernel_regularizer=regularizers.l2(self.alpha))(inp)
        x = layers.Dropout(self.dropout_rate)(x)
        x = layers.Dense(self.units_2, activation="relu", kernel_regularizer=regularizers.l2(self.alpha))(x)
        x = layers.Dropout(self.dropout_rate)(x)
        x = layers.Dense(self.units_3, activation="relu", kernel_regularizer=regularizers.l2(self.alpha))(x)
